...
async function onSubmit(event) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Notify UI layer (resources.js) if it exists
    ...
    if (response.status === 409) {
      const name = payload.resourceName || body?.name || "the given name";
      showFormMessage(
        "info",
        `⛔ Resource already exists.\n\nA resource with the name "${name}" already exists.\n\nPlease:\n• change the name\n• or check the resource list`
      );
      return;
    }
    ...
    if (response.status === 400) {
      if (Array.isArray(body?.errors)) {
        showFormMessage("error", buildValidationMessage(body.errors));
      } else {
        showFormMessage(
          "error",
